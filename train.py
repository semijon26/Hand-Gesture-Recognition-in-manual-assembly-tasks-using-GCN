...
os.makedirs(f"trained_models/{CFG.experiment_name}", exist_ok=True)
...
writer = SummaryWriter(f'training_runs/{CFG.experiment_name}')


def train_eval():
    if CFG.model_type == "LSTM":
        model = lstm.LSTMClassifier(input_dim=CFG.num_feats * 21, hidden_dim=CFG.lstm_hidden_layers,
                                    num_layers=CFG.lstm_num_layers, output_dim=CFG.num_classes,
                                    seq_len=CFG.sequence_length, batch_size=CFG.batch_size)

    if CFG.model_type == "STCONV":
        model = stconv.STConvOpolkaModel(in_channels=CFG.num_feats, spatial_channels=CFG.stconv_spatial_channels,
                                         out_channels=CFG.stconv_out_channels, classes=CFG.num_classes)
    ...
    if CFG.model_type == "MSG3D":
        model = msg3d.Model(num_class=6, num_point=21, num_person=1, num_gcn_scales=13, num_g3d_scales=6,
                            graph=msg3d.AdjMatrixGraph)
    ...
    for epoch in range(start_epoch, CFG.epochs + start_epoch):
        ...
        PATH = f"trained_models/{CFG.experiment_name}/f1{f1_val_micro}_valloss{val_loss}_epoch{epoch}.pth"
        torch.save({
            'epoch': epoch,
